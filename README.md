...
구글 크롬 브라우저의 SpeechRecognition API를 사용하여 실시간 음성 인식으로 성경 구절을 검색하는 Next.js 웹 애플리케이션입니다.

## 주요 기능

- 🎤 **실시간 음성 인식**: Chrome 브라우저의 Web Speech API를 활용한 실시간 음성 인식
- 📖 **성경 구절 검색**: 음성으로 말한 키워드나 책 이름으로 관련 성경 구절 검색
- 🤖 **AI 기반 검색**: OpenAI API를 활용한 지능형 성경 구절 검색
...
이 애플리케이션은 두 가지 방식으로 성경 구절을 검색합니다:

1. **로컬 데이터베이스 검색**: 기본적인 성경 구절 데이터베이스에서 빠르게 검색
2. **OpenAI API 검색**: 로컬에서 결과를 찾지 못한 경우, OpenAI API를 사용하여 더 넓은 범위의 성경 구절을 검색

## 향후 개선 사항

- [ ] 더 많은 성경 구절 데이터베이스 추가
- [ ] 외부 성경 API 연동
- [ ] 검색 결과 공유 기능
- [ ] 즐겨찾기 기능
- [ ] 검색 기록 저장
